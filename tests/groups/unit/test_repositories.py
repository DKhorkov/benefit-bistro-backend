...
@pytest.mark.anyio
async def test_sqlalchemy_groups_repository_get_owner_groups(
        create_test_group: None,
        async_connection: AsyncConnection
) -> None:
    ...
    async_session_factory: async_sessionmaker = async_sessionmaker(bind=async_connection)
    session: AsyncSession = async_session_factory()
    owner_groups: List[GroupModel] = await SQLAlchemyGroupsRepository(session=session).get_owner_groups(
        owner_id=TestGroupConfig.OWNER_ID
    )
    assert len(owner_groups) == 1
    group: GroupModel = owner_groups[0]
    assert group.id == 1
    assert group.owner_id == TestGroupConfig.OWNER_ID
    assert group.name == TestGroupConfig.NAME
...
@pytest.mark.anyio
async def test_sqlalchemy_groups_repository_get_empty_owner_groups(
        map_models_to_orm: None,
        async_connection: AsyncConnection
) -> None:
